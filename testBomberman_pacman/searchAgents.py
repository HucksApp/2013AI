...
class CornersProblem(search.SearchProblem):
  ...
  def isGoalState(self, state):
	...
	"*** YOUR CODE HERE ***"
	return len([x for x in state[2:6] if x[0] == 0]) == len(self.corners)
      ...
      #   hitsWall = self.walls[nextx][nexty]
		...
		if not self.walls[nx][ny]:
			for corner in corners:
				if corner[0] == nx and corner[1] == ny:
					_corner.append((0,0))
				else:
					_corner.append(corner)
			nextState = ((nx, ny) + tuple(_corner))
...
def cornersHeuristic(state, problem):
  ...
  _corners = [x for x in state[2:6] if x[0]!=0]
  
  if len(_corners) == 0:
	return 0
  
  manhattan = [abs(x[0]-state[0])+abs(x[1]-state[1]) for x in _corners]
  h = min(manhattan)

  if len(manhattan) == 4:
	h = h + 2*(walls.height -2) + walls.width -2
  elif len(manhattan) == 3:
	h = h + walls.height -2 + walls.width -2
  elif len(manhattan) == 2:
	h = h + walls.height -2
